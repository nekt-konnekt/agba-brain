...
try {
  ...
  const reasoningText = await reasoning.text(); if (reasoning.status !== 201) fail("reasoning", reasoningText); const result = JSON.parse(reasoningText); if (!result.reasoning?.confidence_reason || !result.reasoning?.severity_reason) fail("reasoning explanations", result); if (!Array.isArray(result.evidence) || result.evidence.length < 1) fail("evidence links", result); if (result.reasoning?.provider !== "openai") fail("reasoning provider", result); pass("OpenAI reasoning with confidence/severity explanations");
