...
try {
  ...
  const briefingText = await briefing.text(); if (briefing.status !== 201) fail("CEO briefing", briefingText); const briefingData = JSON.parse(briefingText); if (!briefingData.briefing?.summary || !Array.isArray(briefingData.items)) fail("CEO briefing output", briefingData); if (briefingData.provider !== "openrouter") fail("briefing provider", briefingData); pass(`CEO briefing generation (${briefingData.model ?? "openrouter/free"})`);
