...
try {
  ...
  const reasoningText = await reasoning.text(); if (reasoning.status !== 201) fail("reasoning", reasoningText); const result = JSON.parse(reasoningText); if (!result.reasoning?.confidence_reason || !result.reasoning?.severity_reason) fail("reasoning explanations", result); if (!Array.isArray(result.evidence) || result.evidence.length < 1) fail("evidence links", result); pass("Gemini reasoning with confidence/severity explanations");
  ...
  const briefingText = await briefing.text(); if (briefing.status !== 201) fail("CEO briefing", briefingText); const briefingData = JSON.parse(briefingText); if (!briefingData.briefing?.summary || !Array.isArray(briefingData.items)) fail("CEO briefing output", briefingData); if (briefingData.provider !== "gemini") fail("CEO briefing provider", briefingData); pass("CEO briefing generation");
