...
const marker=`proposal-execution-e2e-${crypto.randomUUID()}`;
const org=(await rest("agba_organizations",{method:"POST",headers:{Prefer:"return=representation"},body:JSON.stringify({name:`Agba E2E ${marker}`,slug:marker})}))[0]?.id;
if(!org)throw new Error("temporary organization creation failed");
const secret=await rpc("agba_telegram_worker_secret",{});if(!secret)throw new Error("worker secret unavailable");
let actionRow:any=null;let proposalId:string|null=null;let approvalId:string|null=null;
try{
 const action=await rpc("agba_mutate_action",{p_operation:"create",p_action_id:null,p_organization_id:org,p_created_by:null,p_description:`PROPOSAL-EXECUTION-E2E-${crypto.randomUUID()}`,p_owner_name:"Chinedu",p_deadline:null,p_status:"open",p_priority:"high",p_metadata:{test_only:true}});actionRow=Array.isArray(action)?action[0]:action;if(!actionRow?.id)throw new Error("action creation failed");
 ...
 if(actionRow?.id)await rpc("agba_mutate_action",{p_operation:"status",p_action_id:actionRow.id,p_organization_id:org,p_created_by:null,p_description:null,p_owner_name:null,p_deadline:null,p_status:"cancelled",p_priority:null,p_metadata:{test_cleanup:true}}).catch(()=>{});
 if(actionRow?.id)await rest(`agba_actions?id=eq.${actionRow.id}`,{method:"DELETE"}).catch(()=>{});
 await rest(`agba_organizations?id=eq.${org}`,{method:"DELETE"}).catch(()=>{});
}
