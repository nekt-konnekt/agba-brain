(() => {
  ...
  function injectStyle() {
    ...
    style.textContent = `
      ...
      .v2-metrics{display:grid;grid-template-columns:repeat(4,1fr);border:1px solid #2a323b;border-radius:14px;overflow:hidden}.v2-metric{padding:16px 17px;border-right:1px solid #2a323b}.v2-metric:last-child{border-right:none}.v2-metric .label{font-size:9px;letter-spacing:.11em;color:#78838c;font-weight:800}.v2-metric .value{font-family:'Manrope',sans-serif;font-size:22px;font-weight:800;letter-spacing:-.045em;margin-top:10px}.v2-metric .sub{font-size:9px;color:#69747d;margin-top:4px}.v2-metric.accent .value{color:#c6f52d}
      .v2-getting-started{display:grid;gap:1px;border:1px solid #2a323b;border-radius:14px;overflow:hidden}.v2-gs-item{display:flex;align-items:center;gap:12px;background:#151b22;padding:14px 16px}.v2-gs-num{width:22px;height:22px;flex:none;border-radius:50%;border:1px solid #2a323b;display:grid;place-items:center;font-size:11px;color:#78838c}.v2-gs-body{flex:1}.v2-gs-body strong{display:block;font-size:12px;color:#e3e8e3}.v2-gs-body span{display:block;font-size:10px;color:#7e8890;margin-top:2px}.v2-gs-btn{border:1px solid #2a323b!important;background:#171e25!important;border-radius:9px;padding:7px 12px}
      ...
      @media(max-width:1050px){.office-v2-top,.v2-business{grid-template-columns:1fr}.v2-metrics{grid-template-columns:repeat(2,1fr)}.v2-metrics .v2-metric{border-bottom:1px solid #2a323b}.v2-metrics .v2-metric:nth-last-child(-n+2){border-bottom:none}.v2-depts{grid-template-columns:repeat(2,1fr)}}
  ...
  function render(data) {
    ...
    // A brand-new company with zero departments has nothing real to put in
    // four metric cards — every one reads "Not reported yet" / "0". Rather
    // than greet a first-time CEO with a wall of empty stats (the exact
    // "cumbersome dashboard" feeling we're trying to avoid), show a short,
    // concrete two-step path instead. Once a department exists, the real
    // metrics take over automatically.
    const metricsBlock = departments.length > 0 ? `<div class="v2-metrics"><article class="v2-metric"><div class="label">REVENUE</div><div class="value">${money(revenue?.value, currency)}</div><div class="sub">${revenue?.measured_on ? `Measured ${date(revenue.measured_on)}` : 'Not reported yet'}</div></article><article class="v2-metric accent"><div class="label">OUTSTANDING</div><div class="value">${money(outstanding?.value, currency)}</div><div class="sub">Receivables / unpaid</div></article><article class="v2-metric"><div class="label">OPEN MOVES</div><div class="value">${actions.length}</div><div class="sub">Tracked actions in progress</div></article><article class="v2-metric"><div class="label">OPEN RISKS</div><div class="value">${openIssues}</div><div class="sub">Active risks & issues</div></article></div>` : `<div class="v2-getting-started"><div class="v2-gs-item"><span class="v2-gs-num">1</span><div class="v2-gs-body"><strong>Create your first department</strong><span>Sales, Operations — whatever fits how you run things.</span></div><button class="v2-link v2-gs-btn" data-gs="departments" type="button">Add one</button></div><div class="v2-gs-item"><span class="v2-gs-num">2</span><div class="v2-gs-body"><strong>Invite your team</strong><span>Each head reports straight to Agba on Telegram.</span></div><button class="v2-link v2-gs-btn" data-gs="team" type="button">Invite</button></div></div>`;
    ...
    v2.innerHTML = `
      ...
      ${metricsBlock}
      <div class="v2-feed"><section class="v2-card v2-panel"><div class="v2-head"><div><div class="v2-eyebrow">WHAT'S HAPPENING</div><h3>Needs your attention, and what changed</h3><p>One ranked list, so you're not reconciling two panels to see the whole picture.</p></div><button class="v2-link" data-view="actions">All actions</button></div><div class="v2-attention">${attention.length || changes.length ? `${attention.map((x,i) => `<article class="v2-issue ${esc(x.tone || '')}" data-attention-index="${i}"><div class="v2-issue-icon">${esc(x.icon || '!')}</div><div><strong>${esc(x.title)}</strong><span>${esc(x.text)}</span></div><div class="v2-chevron">›</div></article>`).join('')}${changes.map(c => `<div class="v2-change"><strong>${esc(c.title)}</strong><span>${esc(c.text)}</span></div>`).join('')}` : '<div class="v2-empty">Nothing is currently demanding a CEO decision, and no material changes have been recorded yet. Agba will surface the next thing that matters here.</div>'}</div></section></div>
  ...
  function attachV2Handlers() {
    ...
    document.querySelectorAll('.v2-gs-btn').forEach(el => el.addEventListener('click', () => document.querySelector(`.nav-item[data-view="${el.dataset.gs}"]`)?.click()));
  ...
  async function mount() {
    ...
    // Office V2 previously only ever rendered once, on mount — a change made
    // elsewhere (e.g. creating a department, or Ask Agba producing a new
    // action) calls the legacy loadOffice()/renderOffice(), which never
    // touched this layer. That meant V2's hero, metrics and getting-started
    // checklist could go stale until a full page reload. Exposing a refresh
    // hook here so callers that already know how to reach Supabase can ask
    // V2 to re-render with current data without duplicating that logic.
    window.refreshOfficeV2 = async () => {
      if (!mounted) return;
      const { data: fresh, error: freshError } = await client.functions.invoke('office-read', { body: {} });
      if (!freshError && fresh && !fresh.error) render(fresh);
    };
  }
