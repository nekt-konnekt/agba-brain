create or replace function public.agba_mutate_action(
  ...
  p_priority public.agba_severity default null,
  p_metadata jsonb default '{}'::jsonb,
  p_source_ceo_query_id uuid default null,
  p_source_state_item_id uuid default null
)
...
begin
  ...
  if op='create' then
    ...
    insert into public.agba_actions(organization_id,created_by,owner_name,description,deadline,status,priority,metadata,source_ceo_query_id,source_state_item_id)
    values(p_organization_id,p_created_by,nullif(trim(p_owner_name),''),trim(p_description),p_deadline,coalesce(p_status,'open'),coalesce(p_priority,'medium'::public.agba_severity),coalesce(p_metadata,'{}'::jsonb),p_source_ceo_query_id,p_source_state_item_id)
    returning * into a;
