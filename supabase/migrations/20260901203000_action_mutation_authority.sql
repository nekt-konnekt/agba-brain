create table if not exists public.agba_action_history (
...
create index if not exists idx_agba_action_history_action on public.agba_action_history(action_id,created_at desc);

create or replace function public.agba_mutate_action(
  p_operation text,
  ...
  p_priority agba_severity default null,
  p_metadata jsonb default '{}'::jsonb
)
returns setof public.agba_actions
language plpgsql security definer set search_path=public
as $$
declare a public.agba_actions; before_state jsonb; op text:=lower(trim(coalesce(p_operation,'')));
begin
  ...
  if op='create' then
    ...
    values(p_organization_id,p_created_by,nullif(trim(p_owner_name),''),trim(p_description),p_deadline,coalesce(p_status,'open'),coalesce(p_priority,'medium'::agba_severity),coalesce(p_metadata,'{}'::jsonb))
    returning * into a;
    insert into public.agba_action_history(action_id,organization_id,changed_by,operation,before_state,after_state) values(a.id,a.organization_id,p_created_by,'create','{}'::jsonb,to_jsonb(a));
  ...
  if op='status' then
    ...
    update public.agba_actions set owner_name=nullif(trim(p_owner_name),''),updated_at=now() where id=a.id returning * into a;
  ...
  insert into public.agba_action_history(action_id,organization_id,changed_by,operation,before_state,after_state) values(a.id,a.organization_id,p_created_by,op,before_state,to_jsonb(a));
  return next a;
end;
$$;
