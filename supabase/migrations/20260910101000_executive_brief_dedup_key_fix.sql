...
begin
 ...
 select count(*) into v_attention from (select lower(regexp_replace(title,'^(Overdue action:|Overdue:)\\s*','','i')) k from public.agba_briefing_items where briefing_id=v_id and type in ('issue','attention','task','decision','watch') and title !~* 'TELEGRAM-CONNECTOR-E2E-' group by 1) q;
 ...
 select count(*) into v_overdue from (select lower(regexp_replace(title,'^(Overdue action:|Overdue:)\\s*','','i')) k from public.agba_briefing_items where briefing_id=v_id and type in ('issue','attention','task') and (title ~* 'overdue' or content ~* 'overdue') and title !~* 'TELEGRAM-CONNECTOR-E2E-' group by 1) q;
...
begin
 ...
 for r in select type,title,content,priority from public.agba_briefing_items where briefing_id=v_briefing_id and title !~* 'TELEGRAM-CONNECTOR-E2E-' and content !~* 'TELEGRAM-CONNECTOR-E2E-' order by priority asc,created_at asc loop
  v_key:=lower(regexp_replace(coalesce(r.title,''),'^(Overdue action:|Overdue:)\\s*','','i')); if v_key=any(seen_keys) then continue; end if; seen_keys:=array_append(seen_keys,v_key);
