-- E2E-only queue fixtures must not be claimable by production cron workers.
-- Test workers use an e2e-* worker id and fixtures carry payload.test_only=true.
...
begin
  ...
  with candidate as (
    ...
    and ((p_worker_id like 'e2e-%' and coalesce(payload->>'test_only','false')='true') or (p_worker_id not like 'e2e-%' and coalesce(payload->>'test_only','false')<>'true'))
