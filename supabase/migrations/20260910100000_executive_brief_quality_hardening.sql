-- Executive briefing quality hardening: keep internal test artifacts out of CEO delivery,
-- deduplicate the same business commitment across sections, and render executive dates.

create or replace function public.agba_sanitize_briefing_item()
returns trigger language plpgsql security definer set search_path = public as $$
declare v_text text;
begin
  if coalesce(new.title,'') ~* '^(Action outcome:\s*)?TELEGRAM-CONNECTOR-E2E-' then return null; end if;
...
declare
  ...
  v_issues integer := 0; v_overdue integer := 0; v_decisions integer := 0; v_attention integer := 0; v_summary text;
begin
  if v_id is null then return coalesce(new, old); end if;
  select count(*) into v_attention from (
    select lower(regexp_replace(title, '^Overdue:\s*', '', 'i')) k from public.agba_briefing_items
    where briefing_id=v_id and type in ('issue','attention','task','decision','watch') and title !~* 'TELEGRAM-CONNECTOR-E2E-' group by 1
  ) q;
  ...
  select count(*) into v_overdue from (
    select lower(regexp_replace(title, '^Overdue:\s*', '', 'i')) k from public.agba_briefing_items
    where briefing_id=v_id and type in ('issue','attention','task') and (title ~* 'overdue' or content ~* 'overdue') and title !~* 'TELEGRAM-CONNECTOR-E2E-' group by 1
  ) q;
...
returns trigger language plpgsql security definer set search_path = public as $$
declare
  v_briefing_id uuid; v_summary text; v_text text := '🧠 Agba — Morning Brief'; v_section text := ''; v_key text; r record; seen_keys text[] := '{}';
begin
  if coalesce(new.payload->>'type','') <> 'daily_briefing' then return new; end if;
  v_briefing_id := nullif(new.payload->>'briefing_id','')::uuid; if v_briefing_id is null then return new; end if;
  ...
  for r in select type,title,content,priority from public.agba_briefing_items where briefing_id=v_briefing_id and title !~* 'TELEGRAM-CONNECTOR-E2E-' and content !~* 'TELEGRAM-CONNECTOR-E2E-' order by priority asc,created_at asc loop
    v_key := lower(regexp_replace(coalesce(r.title,''), '^Overdue:\s*', '', 'i'));
    if v_key = any(seen_keys) then continue; end if; seen_keys := array_append(seen_keys,v_key);
