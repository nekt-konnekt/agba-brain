...
async function workerSecret(sb: any) {
  const { data, error } = await sb
    ...
    .eq("name", "agba_telegram_worker_secret")
    .maybeSingle();
...
Deno.serve(async (req) => {
  ...
  // Recover work that was claimed by a worker that died or timed out.
  await sb
    .from("agba_telegram_update_inbox")
    .update({ status: "received", last_error: "worker_lease_expired" })
  ...
  if (selectError) {
    console.error("telegram_worker_select_failed", selectError);
    return json({ error: "queue_select_failed" }, 500);
  }
  ...
  if (claimError) {
    console.error("telegram_worker_claim_failed", claimError);
