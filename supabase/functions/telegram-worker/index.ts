...
async function dispatch(update: any, secret: string, inboxId: string) {
  ...
  try { if (JSON.parse(actionBody)?.handled) return; } catch {}
...
Deno.serve(async (req) => {
  ...
  await sb.from("agba_telegram_update_inbox").update({ status: "received", last_error: "worker_lease_expired" })
    .eq("status", "processing").lt("locked_at", new Date(Date.now() - 2 * 60_000).toISOString());
  ...
  if (currentAttempts >= MAX_ATTEMPTS) {
    const { error } = await sb.from("agba_telegram_update_inbox").update({ status: "dead", locked_at: null, last_error: `max_attempts_exceeded:${MAX_ATTEMPTS}` }).eq("id", item.id);
    if (error) { console.error("telegram_worker_dead_letter_update_failed", error); return json({ error: "queue_terminal_update_failed" }, 500); }
    return json({ ok: true, processed: 0, dead_lettered: true, update_id: item.telegram_update_id, attempts: currentAttempts });
  }

  const nextAttempt = currentAttempts + 1;
  const { data: claimed, error: claimError } = await sb.from("agba_telegram_update_inbox").update({ status: "processing", attempts: nextAttempt, locked_at: new Date().toISOString(), last_error: null })
    .eq("id", item.id).in("status", ["received", "failed"]).select("id,attempts,payload").maybeSingle();
  ...
  try {
    ...
    const { error } = await sb.from("agba_telegram_update_inbox").update({ status: "dispatched", dispatched_at: new Date().toISOString(), locked_at: null, last_error: null }).eq("id", claimed.id);
    if (error) throw error;
    ...
    const { error: updateError } = await sb.from("agba_telegram_update_inbox").update({ status: finalFailure ? "dead" : "failed", locked_at: null, last_error: finalFailure ? `max_attempts_exceeded:${MAX_ATTEMPTS};${message}`.slice(0, 2000) : message.slice(0, 2000) }).eq("id", claimed.id);
    if (updateError) console.error("telegram_worker_failure_state_update_failed", updateError);
