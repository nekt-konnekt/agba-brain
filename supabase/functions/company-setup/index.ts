...
type DepartmentInput = {
  ...
  slug?: string;
  description?: string;
  head: { full_name: string; email: string };
};

type SetupInput = {
  company: { name: string; slug: string; timezone?: string; currency_code?: string };
  ceo: { full_name: string };
...
Deno.serve(async (req) => {
  ...
  const e2eMode = Deno.env.get("AGBA_E2E_MODE") === "true";
  ...
  try {
    ...
    const { data: existingCaller, error: existingError } = await admin.from("agba_users").select("id, organization_id, role_id, active").eq("auth_user_id", callerAuthId).maybeSingle();
    ...
    if (!Array.isArray(departments) || departments.length < 1) throw new Error("At least one department is required");
    ...
    const { data: roles, error: rolesError } = await admin.from("agba_roles").select("id, code").in("code", ["ceo", "department_head"]);
    ...
    const { data: organization, error: orgError } = await admin.from("agba_organizations").insert({ name: companyName, slug: companySlug, timezone: input.company.timezone?.trim() || "Africa/Lagos", currency_code: (input.company.currency_code?.trim() || "NGN").toUpperCase() }).select("id, name, slug, timezone, currency_code").single();
    if (orgError) throw orgError;

    const createdHeadIds: string[] = [];
    try {
      const { data: ceo, error: ceoError } = await admin.from("agba_users").insert({ organization_id: organization.id, auth_user_id: callerAuthId, role_id: ceoRole.id, department_id: null, full_name: ceoName, email: authData.user.email ?? null, active: true }).select("id, full_name, email").single();
      ...
      for (const department of normalizedDepartments) {
        ...
        if (useE2EProvisioning) {
          ...
          const { data: invite, error: inviteError } = await admin.auth.admin.inviteUserByEmail(department.head.email, { data: { full_name: department.head.fullName, agba_role: "department_head" } });
          if (inviteError) throw inviteError;
          if (!invite.user) throw new Error(`Could not provision ${department.head.email}`);
          headUserId = invite.user.id;
        }
      ...
      const { error: completeError } = await admin.from("agba_organizations").update({ setup_completed_at: new Date().toISOString(), setup_completed_by: ceo.id }).eq("id", organization.id);
      if (completeError) throw completeError;
      return json(201, { organization, ceo, departments: createdDepartments, setup_completed: true });
    } catch (error) {
      for (const authUserId of createdHeadIds) await admin.auth.admin.deleteUser(authUserId);
