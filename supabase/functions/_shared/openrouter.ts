...
export async function callOpenRouterJson(
    ...
    for (let retry = 0; retry <= maxRetriesPerModel; retry++) {
      ...
      try {
        const response = await fetchImpl("https://openrouter.ai/api/v1/chat/completions", {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}`, "HTTP-Referer": "https://agba-brain.vercel.app", "X-Title": "Agba" },
        ...
        if (!response.ok) {
          ...
          if ((response.status === 429 || response.status >= 500) && retry < maxRetriesPerModel) { await sleep(150); continue; }
          break;
        }
        let payload: any;
        try { payload = JSON.parse(text); } catch { attempts.push({ model, reason: "invalid_provider_response_json" }); break; }
        const content = payload?.choices?.[0]?.message?.content;
        let value: unknown;
        try { value = typeof content === "string" ? JSON.parse(content) : content; } catch { attempts.push({ model, reason: "invalid_model_json" }); break; }
        if (!validator(value)) { attempts.push({ model, reason: "invalid_schema" }); break; }
        return { value, payload, model, attempts };
      } catch (error) {
        const timedOut = error instanceof DOMException && error.name === "AbortError";
        attempts.push({ model, reason: timedOut ? "timeout" : "network_error" });
        if (retry < maxRetriesPerModel) { await sleep(150); continue; }
        break;
  ...
  throw new OpenRouterError(
    ["invalid_schema", "invalid_model_json", "invalid_provider_response_json"].includes(last?.reason ?? "") ? "openrouter_invalid_output" : "openrouter_unavailable",
    "All configured OpenRouter attempts failed", 502, attempts,
  );
}
