...
function configuredModels(explicitModel?: string): string[] {
  const configured = Deno.env.get("PUTER_MODELS")?.split(",").map((model) => model.trim()).filter(Boolean) ?? [];
...
export async function callPuterJson(prompt: string, validator: Validator, options: { fetchImpl?: FetchLike; timeoutMs?: number; maxRetries?: number; model?: string } = {}) {
      ...
      try {
        const response = await fetchImpl(`${baseUrl}/chat/completions`, {
          ...
          body: JSON.stringify({
            ...
            messages: [
              { role: "system", content: "You are Agba, a company's operating brain. Reason only from supplied evidence. Do not invent facts. Return only valid JSON matching the requested schema. Do not wrap JSON in markdown fences or include commentary." },
        ...
        if (!response.ok) {
          ...
          if (retry < maxRetries && retryableStatus(response.status)) { await new Promise((resolve) => setTimeout(resolve, 250 * (retry + 1))); continue; }
          break;
        }
        let payload: any;
        try { payload = JSON.parse(text); } catch { attempts.push({ model: `puter/${model}`, reason: "invalid_provider_response_json" }); break; }
        const value = extractJsonContent(payload);
        if (!validator(value)) { attempts.push({ model: `puter/${model}`, reason: "invalid_model_json" }); break; }
        ...
        if (retry < maxRetries) { await new Promise((resolve) => setTimeout(resolve, 250 * (retry + 1))); continue; }
        break;
  ...
  throw new PuterAIError(last?.reason === "invalid_model_json" ? "puter_invalid_output" : "puter_unavailable", "Puter AI gateway failed to return valid Agba output", 502, attempts);
}
