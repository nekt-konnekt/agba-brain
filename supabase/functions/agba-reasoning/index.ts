...
Deno.serve(async(req)=>{
 ...
 const reportIds=body.evidence.map((e:any)=>e.report_id).filter(Boolean);let reportContext="";
 if(reportIds.length){const {data:reports,error:reportsError}=await admin.from("agba_reports").select("id, raw_text, department_id, created_at").in("id",reportIds).eq("organization_id",body.organization_id);if(reportsError)return json({error:"evidence_lookup_failed",detail:reportsError.message},400);if(!reports||reports.length!==reportIds.length)return json({error:"evidence_not_found"},400);reportContext=reports.map((r:any)=>`REPORT ${r.id}\n${r.raw_text}`).join("\n\n");}
