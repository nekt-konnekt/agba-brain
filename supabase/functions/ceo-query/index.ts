...
function coerce(value:any){
  ...
  const actions=Array.isArray(source.actions)?source.actions.map((a:any)=>{let priority=String(a.priority??a.severity??"medium").toLowerCase();if(priority==="urgent"||priority==="severe")priority="high";if(!["low","medium","high","critical"].includes(priority))priority="medium";return {description:String(a.description??a.action??a.title??a.task??"").trim().slice(0,1000),owner_name:a.owner_name??a.owner??null,deadline:a.deadline??null,priority};}).filter((a:any)=>a.description):[];
...
Deno.serve(async(req)=>{
  ...
  const {data:state,stateError}=await admin.from("agba_state_items").select("id,department_id,kind,state_key,title,summary,status,confidence,severity,recommended_action,first_seen_at,last_seen_at,source_report_id").eq("organization_id",body.organization_id).in("status",["active","monitoring"]).order("last_seen_at",{ascending:false}).limit(40);
  if(stateError) return json({error:"state_lookup_failed",detail:stateError.message},400);
  const {data:reports,reportsError}=await admin.from("agba_reports").select("id,department_id,raw_text,created_at").eq("organization_id",body.organization_id).order("created_at",{ascending:false}).limit(20);
  if(reportsError) return json({error:"report_lookup_failed",detail:reportsError.message},400);
  const {data:openActions,openActionsError}=await admin.from("agba_actions").select("id,owner_name,description,deadline,status,priority,created_at").eq("organization_id",body.organization_id).in("status",["open","in_progress"]).order("created_at",{ascending:false}).limit(30);
  ...
  let result;
  try{result=await callAgbaJson(prompt,()=>true);}catch(error){if(error instanceof AIGatewayError)return json({error:error.code,detail:{message:error.message,attempts:error.attempts}},502);return json({error:"ai_gateway_unavailable"},502);}
  ...
  const actionRows=answer.actions.map((a:any)=>({organization_id:body.organization_id,created_by:actor.id,owner_name:a.owner_name,description:a.description,deadline:a.deadline,status:"open",priority:a.priority,source_ceo_query_id:query.id,metadata:{created_from:"ceo-query"}}));
  let createdActions:any[]=[];
  if(actionRows.length){const {data:actions,error:actionsError}=await admin.from("agba_actions").insert(actionRows).select("*");if(actionsError)return json({error:"action_persist_failed",detail:actionsError.message},400);createdActions=actions??[];}
  ...
  return json({query,answer:{...answer,provider:result.provider??"unknown",model:result.model??"unknown"},actions:createdActions,provenance:{state_items:(state??[]).length,recent_reports:(reports??[]).length,open_actions:(openActions??[]).length}},201);
});
