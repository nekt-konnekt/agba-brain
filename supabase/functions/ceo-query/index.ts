...
Deno.serve(async(req)=>{
  ...
  const stateContext=operationalState.map((s:any)=>`STATE ${s.id}\nkind=${s.kind}\nkey=${s.state_key}\ntitle=${s.title}\nsummary=${s.summary}\nstatus=${s.status}\nconfidence=${s.confidence}\nseverity=${s.severity??"null"}\nrecommended_action=${s.recommended_action??"null"}\nfirst_seen=${s.first_seen_at}\nlast_seen=${s.last_seen_at}\nsource_report=${s.source_report_id??"null"}`).join("\n\n")||"No active operational persistent state.";
  const historyContext=completedActionMemory.map((s:any)=>`COMPLETED ACTION MEMORY ${s.metadata?.action_id??s.id}\ntitle=${s.title}\nsummary=${s.summary}\nowner=${s.metadata?.owner_name??"unassigned"}\ncompleted_at=${s.metadata?.completed_at??s.last_seen_at}`).join("\n\n")||"No completed management-action history.";
  const reportContext=(reports??[]).map((r:any)=>`REPORT ${r.id}\ncreated_at=${r.created_at}\n${r.raw_text}`).join("\n\n")||"No recent reports.";
  const actionContext=(openActions??[]).map((a:any)=>`OPEN ACTION ${a.id}\nstatus=${a.status}\npriority=${a.priority}\nowner=${a.owner_name??"unassigned"}\ndeadline=${a.deadline??"none"}\ndescription=${a.description}`).join("\n\n")||"No open actions.";
  const prompt=`You are Agba, the operating brain of a company. Answer the CEO directly. Use operational persistent state, recent reports, and existing open actions. Completed action history is provided separately for historical questions. Prefer newer evidence when facts conflict. Identify trends, unresolved problems, changes, and management priorities. Never invent facts. Distinguish confirmed facts from inference. Do not give generic management advice when the company evidence is sufficient.\n\nIMPORTANT ACTION-MEMORY RULES:\n- OPEN ACTIONS represent current management work and may be reused only when they clearly cover the same operational incident or intent.\n- COMPLETED ACTION MEMORY is historical evidence only. A completed action does NOT satisfy, resolve, or cover a new unresolved incident merely because the wording or subject is similar.\n- If the CEO describes a new incident, deadline, order, customer, quantity, or blocker and no open action clearly covers that same incident, recommend a NEW action even if a similar completed action exists in history.\n- Never cite a completed historical action as the current action for a new incident.\n\nCEO QUESTION:\n${question}\n\nOPERATIONAL PERSISTENT STATE:\n${stateContext}\n\nCOMPLETED ACTION HISTORY:\n${historyContext}\n\nRECENT REPORTS:\n${reportContext}\n\nOPEN ACTIONS:\n${actionContext}\n\nReturn ONLY valid JSON with exactly these fields: {"answer":"direct CEO answer","confidence":"high|medium|low","confidence_reason":"evidence-based reason","signals":[{"signal":"short business signal","evidence":"specific supporting evidence"}],"actions":[{"description":"specific action Agba recommends","owner_name":"person or null","deadline":"ISO timestamp or null","priority":"low|medium|high|critical"}]}. If the CEO asks what needs attention, what should be done, how to respond, what to prioritize, or otherwise asks for management action, and the evidence supports a concrete action, you MUST include at least one action. Use the strongest evidence-backed management action, not generic advice. Do not create an action merely because an existing OPEN action already covers the same operational intent. If an existing OPEN action does not cover the same incident, create a new action. Historical completed actions must never be used as a reason to omit a new action for a distinct unresolved incident. Only omit actions when no concrete management action is supported by the evidence. Keep answer concise but substantive.`;
  ...
  if(!managementActions.length&&isManagementQuestion(question)){
    const evidenceActions=operationalState.filter((s:any)=>String(s.recommended_action??"").trim()).sort((a:any,b:any)=>{const severity=priorityRank(String(b.severity??"medium").toLowerCase())-priorityRank(String(a.severity??"medium").toLowerCase());if(severity!==0)return severity;return String(b.last_seen_at??"").localeCompare(String(a.last_seen_at??""));}).map((s:any)=>({description:String(s.recommended_action).trim().slice(0,1000),owner_name:null,deadline:null,priority:["critical","high","medium","low"].includes(String(s.severity??"").toLowerCase())?String(s.severity).toLowerCase():"medium"}));
