...
Deno.serve(async (req) => {
  ...
  const { data: actor, error: actorError } = await admin
    .from("agba_users")
    .select("id, organization_id, department_id, active, full_name, agba_roles(code)")
  ...
  const openRisks = state.filter((s: any) => ["risk", "issue"].includes(s.kind));
  const critical = openRisks.filter((s: any) => ["critical", "high"].includes(s.severity)).slice(0, 5);
  const changes = state.filter((s: any) => s.kind === "observation" || s.kind === "opportunity").slice(0, 5);

  const metric = (keys: string[]) => metrics.find((m: any) => keys.some((k) => String(m.key ?? m.name).toLowerCase().includes(k)));
  const revenue = metric(["revenue", "sales"]);
  const outstanding = metric(["outstanding", "receivable", "receivables", "unpaid"]);

  const departmentPulse = departments.map((d: any) => {
    const deptReports = todayReports.filter((r: any) => r.department_id === d.id);
    const deptState = state.filter((s: any) => s.department_id === d.id);
    const risk = deptState.find((s: any) => ["risk", "issue"].includes(s.kind));
    return {
      id: d.id,
      name: d.name,
      health: risk ? "warn" : deptReports.length ? "good" : "missing",
      text: risk ? risk.summary : deptReports.length ? `${deptReports.length} report${deptReports.length > 1 ? "s" : ""} received today` : "No report received today",
    };
  });

  const attention = [
    ...critical.map((s: any) => ({ icon: "!", title: s.title, text: s.summary, badge: String(s.kind).toUpperCase(), tone: ["critical", "high"].includes(s.severity) ? "danger" : "warn", source_state_item_id: s.id })),
  ...
  return json({
    ...
    today,
    reporting_health: health,
    metrics: {
      revenue: revenue ? { value: revenue.value_numeric ?? revenue.value_text, unit: revenue.unit, measured_on: revenue.measured_on } : null,
      outstanding: outstanding ? { value: outstanding.value_numeric ?? outstanding.value_text, unit: outstanding.unit, measured_on: outstanding.measured_on } : null,
    },
    attention,
    changes: changes.map((s: any) => ({ icon: s.kind === "opportunity" ? "+" : "↑", title: s.title, text: s.summary, source_state_item_id: s.id })),
